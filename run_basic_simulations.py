...
#sim = s.simulator(time_factor=True,test_edit_factor=True,full_simulation=True,do_logging=True)
#sim = s.simulator(time_factor=True,full_simulation=True,do_logging=True)
sim = s.simulator(full_simulation=True,do_logging=True)
...
basic_run_sets = (20,50,70,100,160,230,290,350,400,500,650,800)
#basic_run_sets = (500,)
...
n_caught_f_brn = np.ndarray(dtype=float,shape=len(run_sets))
...
max_f = mixed_results[0].caught_failures + mixed_results[0].missed_failures
for i in range(len(run_sets)):
    n_caught_f_std[i] = caught_f_std[i]/max_f
    n_caught_f_plt[i] = caught_f_plt[i]/max_f
    n_caught_f_brn[i] = caught_f_brn[i]/max_f
...
plt.plot(rsets,n_caught_f_std,label = 'Standard')
plt.plot(rsets,n_caught_f_plt,label = 'Platform')
plt.plot(rsets,n_caught_f_brn,label = 'Branch')
plt.plot(rsets,n_caught_f_mix,label = 'Mixed (Branch/Platform)')
plt.ylim(0,1.1)
